...
afs_zmq::v2_decoder_t::v2_decoder_t (size_t bufsize_, int64_t maxmsgsize_) :
...
afs_zmq::v2_decoder_t::~v2_decoder_t ()
{
    int rc = in_progress.close ();
    //errno_assert (rc == 0);
    if (rc == 0) {
        // printf("in progress message is closed, maybe error or going to reconnect\n");
    }
}

int afs_zmq::v2_decoder_t::flags_ready ()
{
    msg_flags = 0;
...
int afs_zmq::v2_decoder_t::one_byte_size_ready ()
{
    //  Message size must not exceed the maximum allowed size.
    if (maxmsgsize >= 0)
        if (afs_unlikely (tmpbuf [0] > static_cast <uint64_t> (maxmsgsize))) {
            errno = EMSGSIZE;
            return -1;
        }
...
int afs_zmq::v2_decoder_t::eight_byte_size_ready ()
{
    ...
    //  Message size must not exceed the maximum allowed size.
    if (maxmsgsize >= 0)
        if (afs_unlikely (msg_size > static_cast <uint64_t> (maxmsgsize))) {
            errno = EMSGSIZE;
            return -1;
        }

    //  Message size must fit into size_t data type.
    if (afs_unlikely (msg_size != static_cast <size_t> (msg_size))) {
        errno = EMSGSIZE;
        return -1;
    }
...
int afs_zmq::v2_decoder_t::message_ready ()
{
