...
int afs_zmq::stream_engine_t::decode_and_push (msg_t *msg_)
{
    ...
    */
    ...
    if (rc == 0) {
	//application finish handle the msg,
	//delete the msg, otherwise will cause memory leak
	    msg_->close();
    }
...
void afs_zmq::stream_engine_t::error ()
{
    ...
    //reconnect by using a new tcp_connector
    //note only tcp_connecter may enable auto_reconnect
    if(auto_reconnect_) { 
        // LOG_MSG("stream engine introduced by tcp_connect disconnect\n");

        //parse the end point address
        std::string uri (endpoint);
        std::string::size_type pos = uri.find("://");
        std::string protocol = uri.substr (0, pos);
        std::string address = uri.substr (pos + 3);
        
        //make new address for tcp_connector by the endpoint address
        int rc = 0;
        ...
        tcp_connecter_t * connecter = 
            new (std::nothrow) tcp_connecter_t (
                wid_,
                in_callback_,
                out_callback_,
                options,
        ...
        connecter->set_poller(io_object_t::get_poller());
        connecter->start_connecting();
        // LOG_MSG("start asynchronously reconneect!\n");
    }

    //unplug the current stream_engine
    unplug ();
    delete this;
}
