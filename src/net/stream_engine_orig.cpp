...
bool zmq::stream_engine_t::in_event ()
{
...
bool zmq::stream_engine_t::out_event ()
{
...
int zmq::stream_engine_t::pull_and_encode (msg_t *msg_)
{
    ...
    if (out_callback_) {
        ...
        //}
        int rc = out_callback_->PullRecord(&data, &len);
        if (data) {
            out_callback_->Ack(data);
            msg_->init_size(len);
            memcpy(msg_->data(), data, len);
            if (rc) {
                //LOG_MSG("pull: rc -2\n");
                return -2;
            }
...
int zmq::stream_engine_t::decode_and_push (msg_t *msg_)
{
    ...
    int rc = in_callback_->PushRecord(msg_->data(), msg_->size(), this);
