...
namespace afs_zmq
{
    ...
    public:
        ...
        inline encoder_base_t (size_t bufsize_, afs::OutCallbackBase* out_callback_) :
            bufsize (bufsize_),
            out_callback(out_callback_),
            in_progress (NULL),
            data (NULL),
            size (0)
        {
        ...
        inline size_t encode (unsigned char **data_, size_t size_)
        {
            ...
            //fprintf(stderr, "    enter encode to_write %d\n", to_write);
            ...
            while (pos < buffersize) {
                ...
                if (!to_write) {
                    if (new_msg_flag) {
                        ...
                        if (data) {
                            //LOG_MSG(" ACK msg\n");
                            out_callback->Ack(data);
                            //LOG_MSG(" after ACK msg\n");
                            data = NULL;
                            size = 0;
                        }
                        //fprintf(stderr, "    clear msg\n");
                        break;
                    }
        ...
        // note
        void load_msg (msg_t *msg_, void* data_, uint32_t size_)
        {
            //fprintf(stderr, "load msg %d\n", to_write);
