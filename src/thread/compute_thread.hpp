...
protected:
    ...
    void EmitData(int dest, OutT& msg) {
        //LOG_MSG("Worker %d with push size %d\n", worker_dest, down_writer_->GetSize(worker_dest));
    ...
    //the API for user worker to clear input, disable backpressure
    void ClearInput() {
        WInT* in_tuple = NULL;
    ...
    int Nonblock_EmitData(int dest, OutT& msg) {
    ...
    int Nonblock_EmitData(int worker_dest, int thr_dest, OutT& msg) {
    ...
    //add three APIs to enable synchronization among multiple threads between two workers
    void EmitData(int worker_dest, int thr_dest, OutT& msg) {

        //LOG_MSG("Worker %d with Ack size %d \n", worker_dest, down_writer_->GetPushSize(worker_dest));
    ...
    void EmitReverseDataQuick(int dest_worker,int dest_thr, ROutT& msg) {
        ...
        slot->set_worker_source(get_wid());
        slot->set_thr_source(get_tid());
        //Note that dest_thr should be a number between 0~compute_thread_num of the received worker, or will case memory error.
        slot->set_thr_dest(dest_thr);
        slot->data() = msg;
