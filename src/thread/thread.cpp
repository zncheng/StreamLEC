...
int64_t afs::ThreadBase::WaitWorker (afs_zmq::command_t* cmd, bool is_block) {

    int64_t rc = -1;
    if (is_block) {
        rc = main_to_thread_.recv(cmd, -1);
        afs_assert(rc==0, "%s calls WaitWorker() error %ld\n",
                thread_str_, rc);
    }
    else {
        rc = main_to_thread_.recv(cmd, 0);
        if (rc != 0) {
            ...
            afs_assert(0, "%s WaitWorker unknown error\n",
                thread_str_);
        }
    }
