...
namespace afs {
    ...
    template <class InT>
        class UpThreadTraceAFS : public UpThread<InT, NullClass> {
            ...
            void Emit(void* data, uint32_t len) {
                ...
                //LOG_MSG("index is %d with address %p\n", index, data);
    ...
    template<class T>
        void UpThreadTraceAFS<T>::ThreadInitHandler() {
            ...
            else if (adapter_type == "ram_d") {
                LOG_MSG("RAM adapter with double is used\n");
                adapter_ = new AdapterRAM_d();
            }
    ...
    template<class T>
        void UpThreadTraceAFS<T>::ThreadMainHandler() {
            ...
            while (1) {
                ...
                //LOG_MSG("after reading string: %p content: %s\n", data, (char*)data);
                if (data == NULL) {
                    break;
                }
            ...
            used_time_ = ((end_time.tv_sec + end_time.tv_usec / 1000000.0) - (start_time.tv_sec + start_time.tv_usec / 1000000.0));
